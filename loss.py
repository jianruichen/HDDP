...
from sklearn.metrics import roc_auc_score, accuracy_score, recall_score, f1_score, roc_curve, precision_score, average_precision_score
...
class ReconLoss(nn.Module):
    ...
    def hyperdeoder(self, z, edge_index, c):
        def FermiDirac(dist):
        ...
        dist = self.manifold.sqdist(z_i, z_j, c)
        return FermiDirac(dist)

    def forward(self, z, pos_edge_index, c, neg_edge_index=None):
        decoder = self.hyperdeoder if self.use_hyperdecoder else self.decoder
        pos_loss = -torch.log(
            decoder(z, pos_edge_index, c) + EPS).mean()
        if neg_edge_index == None:
            neg_edge_index = negative_sampling(pos_edge_index, num_neg_samples=pos_edge_index.size(1) * self.sampling_times)
        neg_loss = -torch.log(1 - decoder(z, neg_edge_index, c) + EPS).mean()

        return pos_loss + neg_loss

    def predict(self, z, pos_edge_index, neg_edge_index, t, c):
        decoder = self.hyperdeoder if self.use_hyperdecoder else self.decoder

        pos_y = z.new_ones(pos_edge_index.size(1)).to(device)
        neg_y = z.new_zeros(neg_edge_index.size(1)).to(device)
        y = torch.cat([pos_y, neg_y], dim=0)
        pos_pred = decoder(z, pos_edge_index, c)
        neg_pred = decoder(z, neg_edge_index, c)
        ...
        roc_old = auc(fpr, tpr)
        return roc_auc, ap, acc, f1
        # return roc_auc, aupr, acc, recall, precision, f1, roc
